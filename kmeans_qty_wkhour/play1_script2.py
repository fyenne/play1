...
df = spark.sql("""select * 
        from dsc_dws.dws_dsc_wh_ou_daily_kpi_sum where operation_day > '20210901' and operation_day < '20220404' """)
...
print(df['operation_day'].max(), df['ou_code'].head(3))
...
def mnb_kmeans_in(ou_code):
        ...
        df_rec = df_sub.query("inbound_receive_qty != 0 & operation_day <  20220301")
        df_re2 = df_sub.query("inbound_receive_qty != 0 & operation_day >= 20220301")
        ...
        df_re2['kernal_core1'] = alg1.predict(np.reshape(list(df_re2['inbound_receive_qty']), (-1,1)))
        df_re2 = df_re2.merge(df_rec[['kernal_core1','kernal_value1']].drop_duplicates(), on = 'kernal_core1')
        df_rec = pd.concat([df_rec, df_re2], ignore_index=True, axis = 0)
        """
        merging
        """
        df_fin = df_fin.append(df_rec).reset_index().drop(['index'], axis = 1)
...
def mnb_kmeans_out(ou_code):
        ...
        df_rec = df_sub.query("outbound_shipped_qty != 0 & operation_day <  20220301")
        df_re2 = df_sub.query("outbound_shipped_qty != 0 & operation_day >= 20220301")
        ...
        df_re2['kernal_core2'] = alg1.predict(
            np.reshape(list(df_re2['outbound_shipped_qty']), (-1,1))
            )
        df_re2 = df_re2.merge(
            df_rec[['kernal_core2','kernal_value2']].drop_duplicates(), on = 'kernal_core2')
        df_rec = pd.concat([df_rec, df_re2], ignore_index=True, axis = 0)
...
def mnb_kmeans_hr(ou_code):
        ...
        df_rec = df_sub.query("total_working_hour != 0 & operation_day <  20220301")
        df_re2 = df_sub.query("total_working_hour != 0 & operation_day >= 20220301")
        ...
        df_re2['kernal_core3'] = alg1.predict(np.reshape(list(df_re2['total_working_hour']), (-1,1)))
        df_re2 = df_re2.merge(df_rec[['kernal_core3','kernal_value3']].drop_duplicates(), on = 'kernal_core3')
        df_rec = pd.concat([df_rec, df_re2], ignore_index=True, axis = 0)
...
print(p, '\n', 'for those who droped during kmeans')
# try: 
#     print(mnb_kmeans_in('AAPXXSHIHS')['ou_code'])
# except:
#     print('try failed')
# ou_codes
"""
...
def mnb_kmeans_hr2(ou_code):
        ...
        df_rec = df_sub.query("outsource_working_hour != 0 & operation_day <  20220301")
        df_re2 = df_sub.query("outsource_working_hour != 0 & operation_day >= 20220301")
        ...
        df_re2['kernal_core4'] = alg1.predict(np.reshape(list(df_re2['outsource_working_hour']), (-1,1)))
        df_re2 = df_re2.merge(df_rec[['kernal_core4','kernal_value4']].drop_duplicates(), on = 'kernal_core4')
        df_rec = pd.concat([df_rec, df_re2], ignore_index=True, axis = 0)
...
shpe1 = df_final.shape
tb = df_final[df_final['operation_day'].astype(int) < 20220301].groupby(
    ['ou_code', 'kernal_core1', 'kernal_core2'], as_index = False
    )['total_working_hour'].quantile(.75).reset_index()
print(tb.shape, "tbshape", tb.columns)
tb.columns = ['ou_code', 'kernal_core1', 'kernal_core2', 'qt_75_wh']

df_final = df_final.merge(tb, on = ['ou_code', 'kernal_core1', 'kernal_core2'], how = 'inner')
try:
    print("check shape",shpe1,"before vs after", df_final.shape)
except:
    pass
...
tb = df_final[df_final['operation_day'].astype(int) < 20220301].groupby(
    ['ou_code', 'kernal_core1', 'kernal_core2']
    )['outsource_working_hour'].quantile(.75).reset_index()

tb.columns = ['ou_code', 'kernal_core1', 'kernal_core2', 'qt_75_os']
...
df_final['min_wh'] = df_final['min_wh'].astype(float)
df_final['max_wh'] = df_final['max_wh'].astype(float)

df_final['kernal_core4'] = df_final['kernal_core4'].astype(int)
df_final[[
    'inbound_receive_qty', 'outbound_shipped_qty', 'total_working_hour' 
    ]]=df_final[[
    'inbound_receive_qty', 'outbound_shipped_qty', 'total_working_hour' 
    ]].astype(np.float32)
...
df_final['inc_day']  = '99991231'
df_final['operation_day']  = df_final['operation_day'].astype(str)
...
spark.sql("""insert overwrite table tmp_dsc_dws.dws_qty_working_hour_labeling_sum_df 
               select ou_code, cast(operation_day as string),inbound_receive_qty
...
#     "inc_day").parquet(
#         "hdfs://dsc/hive/warehouse/dsc/DWS/tmp_dsc_dws/dws_qty_working_hour_labeling_sum_df")
# try:
#     spark.sql("""msck repair table tmp_dsc_dws.dws_qty_working_hour_labeling_sum_df""")
#     spark.sql("""alter table tmp_dsc_dws.dws_qty_working_hour_labeling_sum_df drop partition (inc_day='20210817')""")
# except:
    # pass
